...
class MotherDuck_IbisConnection(BaseIbisConnection):
    ...
    def __init__(self,
        ...
        self.database_backend: str =             CONST_DB_BACKEND.MOTHERDUCK.value
        self.database_abstraction_layer: str =   CONST_DB_ABSTRACTION_LAYER.IBIS.value

        

        self.template_connection_string:    Optional[str] = "md://{USERNAME}:{PASSWORD}@{HOST}:{PORT}/{DATABASE_NAME}"


    def connect_ibis(self, connection_string: str) -> ibis.BaseBackend:
        """Connect to the database using the provided connection string."""

        db_connection = ibis.mssql.connect(connection_string)

        if db_connection is None:
            raise ValueError("MotherDuck_IbisConnection: Connection could not be established")
