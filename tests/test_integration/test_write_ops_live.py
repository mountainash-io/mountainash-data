...
def _assert_rename_table_roundtrip(be, old_name, new_name):
...
def _assert_public_upsert_roundtrip(be, table_name, prepare_target=None):
...
def _add_primary_key(be, table_name):
    be._require_connected()._ibis_conn.raw_sql(
        f"ALTER TABLE {table_name} ADD PRIMARY KEY (id)"
    )


def _prepare_trino_merge_target(be, table_name):
...
def test_rename_table_live_postgres(postgres_backend):
    _assert_rename_table_roundtrip(postgres_backend, "ren_pg_old", "ren_pg_new")


@pytest.mark.integration
def test_rename_table_live_mysql(mysql_backend):
    _assert_rename_table_roundtrip(mysql_backend, "ren_my_old", "ren_my_new")
...
def test_upsert_via_dispatch_postgres(postgres_backend):
    ...
    _assert_public_upsert_roundtrip(
        postgres_backend,
        "up_pg",
        prepare_target=_add_primary_key,
    )
...
def test_upsert_via_dispatch_mssql(mssql_backend):
    ...
    _assert_public_upsert_roundtrip(mssql_backend, "up_mssql")
...
def test_upsert_via_dispatch_trino(trino_backend):
    ...
    _assert_public_upsert_roundtrip(
        trino_backend,
        "up_trino",
        prepare_target=_prepare_trino_merge_target,
    )
...
def test_upsert_via_dispatch_exasol(exasol_backend):
    ...
    _assert_public_upsert_roundtrip(exasol_backend, "up_exasol")


@pytest.mark.integration
def test_upsert_via_dispatch_oracle(oracle_backend):
    """be.upsert() public dispatch — MERGE via generic path (oracle)."""
    _assert_public_upsert_roundtrip(oracle_backend, "up_ora")
